...
public class Grayscale : ImageEffectBase {
    ...
    public Coroutine Fade(bool toDark, float time)
    {
        return StartCoroutine(FadeCoroutine(toDark, time));
    }

    private IEnumerator FadeCoroutine(bool toDark, float time)
    {
        float initialRampOffset = toDark ? 0 : -1;
        float targetRampOffset = toDark ? -1 : 0;
